    ...
    ui(new Ui::OrganizerUI)
{
    ...
    calendar.getMounth()->getDay(QDateTime::currentDateTime().date().day())->setLeft(QTime(0, 5, 0));
...
{
    qDebug() << "done";
        //int ** times = OrganizerUI::get_Date_and_Deadlines();
...

        {
            QString curButtonName = QString("cal_button_%1").arg(i);
            int j;
            for (j = 0; organizer->getGoal(i)->isStepCompleted(j) && j < organizer->getGoal(i)->getStepCount(); j++);
            if (organizer->getGoal(i)->getStepDeadline(j).date().day() >= date && date >= QDateTime::currentDateTime().date().day())
                steps_count++;
        }
        ...
        ui->CalendarActions_Label->setText("Start:" + curDay->getStart().toString() + "\nEnd:" + curDay->getEnd().toString() + "\nTime left:" + curDay->getLeft().toString() + "\nCount tasks:" + QString::number(steps_count));

        for (int k = 0; k < 31; k++)
        {
            steps_count =0;
            QString curButtonName = QString("cal_button_%1").arg(k+6);
            for (int i = 0; i < organizer->getGoalCount(); i++)
            {

                int j;
                for (j = 0; organizer->getGoal(i)->isStepCompleted(j) && j < organizer->getGoal(i)->getStepCount(); j++);
                if (organizer->getGoal(i)->getStepDeadline(j).date().day() >= k && k >= QDateTime::currentDateTime().date().day())
                    steps_count++;
            }

            switch(steps_count){
              case 0:
                ui->Calendar_frame->findChild<QPushButton*>(curButtonName)->setStyleSheet("background-color : rgb(225, 225, 225)");
                break;
              case 1:
                ui->Calendar_frame->findChild<QPushButton*>(curButtonName)->setStyleSheet("background-color : rgb(255,255,0)");
                break;
              case 2:
                ui->Calendar_frame->findChild<QPushButton*>(curButtonName)->setStyleSheet("background-color : rgb(255, 80, 20)");
                break;
              default:
                ui->Calendar_frame->findChild<QPushButton*>(curButtonName)->setStyleSheet("background-color : rgb(255, 20, 50)");
                break;
...
void OrganizerUI::decreaseSecond()
{
    ...
    int index = -1;
    for (int i = 0; i < organizer->getGoal(currentGoal)->getStepCount(); i++)
    {
